...
void HighScoresWidget::update(){
    ...
    if (ranking == 1)
        ...
        yourScore = "You are currently ranked number " + asString(ranking).toUTF8() + ". Almost there!";
    }
    ...
    for(auto& user : top){
        ...
        if(!user.lastGame.isNull())
            table->elementAt(row,4)->addWidget(std::make_unique<WText>(user.lastGame.timeTo(WDateTime::currentDateTime()) + "ago"));
        else
