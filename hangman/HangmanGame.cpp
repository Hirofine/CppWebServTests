...
#include "HighScoresWidget.hpp"

HangmanGame::HangmanGame():WContainerWidget(),game_(0),scores_(0){
    session_.login().changed().connect(this, &HangmanGame::onAuthEvent);
    std::unique_ptr<Auth::AuthModel> authModel = std::make_unique<Auth::AuthModel>(Session::auth(), session_.users());
    authModel->addPasswordAuth(&Session::passwordAuth());
    authModel->addOAuth(Session::oAuth());

    std::unique_ptr<Auth::AuthWidget> authWidget = std::make_unique<Auth::AuthWidget>(session_.login());
    ...
    backToGameAnchor_ = links_->addWidget(std::make_unique<WAnchor>("/play", "Gamin Grounds"));
    ...
    WApplication::instance()->internalPathChanged().connect(this, &HangmanGame::handleInternalPath);
